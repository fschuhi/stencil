...
- **Templates:** `data/templates.yaml` currently lists two artefact type, `data/LLM_INSTRUCTIONS.md.j2` → `LLM_INSTRUCTIONS.md` and `data/CRITICAL_RULES.md.j2` → `CRITICAL_RULES.md`. More entries are expected; the schema doesn't need to change to add them.
